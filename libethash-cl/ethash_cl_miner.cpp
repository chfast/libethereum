...
void ethash_cl_miner::search(uint8_t const* header, uint64_t target, search_hook& hook)
{
	try
	{
		...
		uint32_t const c_notFound = -1;
		m_queue.enqueueWriteBuffer(m_searchBuffer, false, 0, sizeof(uint32_t), &c_notFound);
		...
		for (;; start_nonce += m_globalWorkSize)
		{
			...
			uint32_t* pResult = (uint32_t*)m_queue.enqueueMapBuffer(m_searchBuffer, true, CL_MAP_READ, 0, sizeof(uint32_t));
			uint32_t result = *pResult;
			m_queue.enqueueUnmapMemObject(m_searchBuffer, pResult); // FIXME: Looks like not needed

			if (result != c_notFound)
			{
				uint64_t nonce = start_nonce + result;
				bool exit = hook.found(&nonce, 1);
